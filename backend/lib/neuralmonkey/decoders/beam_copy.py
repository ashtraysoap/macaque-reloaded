...
class SearchResults(NamedTuple(
    ...
    Attributes:
        ...
        parent_ids: A ``(time, batch, beam)``-shaped tensor.
    """
...
class BeamSearchDecoder(ModelPart):
    ...
    def get_initial_loop_state(self) -> BeamSearchLoopState:
        ...
        search_results = SearchResults(
            scores=tf.zeros(
                shape=[1, self.batch_size, self.beam_size],
                dtype=tf.float32,
                name="beam_scores"),
    ...
    def decoding_loop(self) -> BeamSearchOutput:
        ...
        return BeamSearchOutput(
            ...
            attention_loop_states=[{'weights': h.weights,
                                    'contexts': h.contexts}
                                    for h in final_loop_state.decoder_loop_state.histories.attention_histories])
    ...
    def get_body(self) -> Callable[[Any], BeamSearchLoopState]:
        ...
        def body(*args: Any) -> BeamSearchLoopState:
            ...
            next_output = SearchResults(
                ...
                token_ids=append_tensor(search_results.token_ids, next_word_ids),
                parent_ids=append_tensor(search_results.parent_ids, parent_ids))
