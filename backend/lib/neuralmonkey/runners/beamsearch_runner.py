...
class BeamSearchRunner(BaseRunner[BeamSearchDecoder]):
    ...
    class Executable(BaseRunner.Executable["BeamSearchRunner"]):
        ...
        def prepare_results(self, output):
            bs_scores = [s[self.rank - 1] for s in output.last_search_step_output.scores]

            last_search_step_output = output.last_search_step_output
            # The batch_size is always 1, we can eliminate this dimension.
            scores = np.squeeze(last_search_step_output.scores, axis=1)
            token_ids = np.squeeze(last_search_step_output.token_ids, axis=1)
            parent_ids = np.squeeze(last_search_step_output.parent_ids, axis=1)
            beam_size = parent_ids.shape[1]

            # BeamSearchOutputGraph expects values with shape [time, beam].

            # token_ids.shape = [time, beam]
            decoded_tokens = []
            for t in range(token_ids.shape[0]):
                tok_dec = [self.decoder.vocabulary.index_to_word[tok_id]
                    for tok_id in token_ids[t]]
                decoded_tokens.append(tok_dec)

            self.set_result(outputs=[{
                'scores': scores,
                'tokens': decoded_tokens,
                'parent_ids': parent_ids,
                'alignments': output.attention_loop_states[0]['weights']
             }],
